...
Ran `gitleaks detect --source . --verbose` over the full Git history 
(33 commits) before final submission.

**Result**: `no leaks found` — confirmed no API keys, credentials, or secrets 
were ever committed to source control at any point in the project's history.